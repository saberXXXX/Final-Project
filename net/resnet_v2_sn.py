...
def resnetv2sn50(pretrained = True, **kwargs):
    ...
    if pretrained:
        checkpoint = torch.load('./net/pretrained/ResNet50v2+SN(8,32)-77.57.pth',map_location='cpu')
...
def resnetv2sn101(pretrained = True, **kwargs):
    ...
    if pretrained:
        checkpoint = torch.load('./models/ResNet101v2+SN(8,32).pth',map_location='cpu')
        wrong = list(checkpoint['state_dict'].keys())
        right = []
        for i in range(len(wrong)):
            right.append(wrong[i][7:])
